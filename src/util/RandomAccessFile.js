...
export class RandomAccessFile {
    ...
    copy(length) {
        ...
        this.seek(fs.readSync(this.fd, buffer, 0, buffer.length, this.offset));
        return buffer;
    }

    read(length = this.available) {
        return new ByteBuffer(this.copy(length));
    }
    ...
    write(data) {
        this.seek(fs.writeSync(this.fd, data, 0, data.length, this.offset));
    }
};
